        ...
        <div class="row g-4">
            <div class="col-sm-12 col-xl-12">
                <div class="bg-secondary rounded h-100 p-4">
                    ...
                    <form method="POST" action="{{ route('store-parcour-maitre') }}">
                        @csrf
                        <div class="mb-3">
                            <label for="" class="form-label">Maître</label>
                            <select class="form-select" id="floatingSelect"
                                aria-label="Floating label select example" name="maitre_id">
                                <option disabled selected>Selectionnez le maître</option>
                                @foreach ($maitres as $maitre)
                                    <option value="{{ $maitre->id }}">{{ $maitre->nom }} {{ $maitre->prenoms }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
